...
mod zks_provider_tests {
    ...
    async fn test_provider_estimate_fee() {
        ...
        let estimated_fee = provider.estimate_fee(transaction).await;
    ...
    async fn test_provider_estimate_gas_l1_to_l2() {
        ...
        let estimated_fee = provider.estimate_gas_l1_to_l2(transaction).await;
    ...
    async fn test_provider_get_all_account_balances() {
        let provider = era_provider();
        let zk_wallet =
            ZKSWallet::new(local_wallet().clone(), None, Some(provider.clone()), None).unwrap();
        ...
        let balances = provider
            .get_all_account_balances(zk_wallet.l2_address())
            .await
            .unwrap();
